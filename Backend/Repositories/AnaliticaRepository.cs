...
namespace ApiAnalitica.Infrastructure.Repositories
{
    public class AnaliticaRepository : IAnaliticaRepository
    {
        ...
        public async Task<DashboardGerenteResponse> GetGerenteData(DateTime inicio, DateTime fin)
        {
            ...
            decimal vAnt = await connection.ExecuteScalarAsync<decimal>(
                @"SELECT COALESCE(SUM(monto), 0) FROM ventas WHERE etapa = 'Ganado' AND fecha_venta BETWEEN @inicioAnt AND @finAnt", param);
            ...
            var emb = await connection.QueryAsync<EmbudoDatoDTO>(
                @"SELECT etapa as Etiqueta, COUNT(*) as Cantidad, 
                ROUND(COUNT(*) * 100.0 / NULLIF((SELECT COUNT(*) FROM ventas WHERE fecha_venta BETWEEN @inicio AND @fin), 0), 2) as Porcentaje 
                FROM ventas WHERE fecha_venta BETWEEN @inicio AND @fin 
                GROUP BY etapa", param);
            res.EmbudoVentas = emb.ToList();

            var sups = await connection.QueryAsync<SupervisorDesempenoDTO>(
                @"SELECT sup.id, sup.nombre as Supervisor, 
                COALESCE(SUM(CASE WHEN v.etapa = 'Ganado' THEN v.monto ELSE 0 END), 0) as VentasGanadas, 
                COALESCE(AVG(CASE WHEN v.etapa = 'Ganado' AND v.csat > 0 THEN v.csat ELSE NULL END), 0) as CsatPromedio, 
                COALESCE((SELECT origen_campana FROM ventas v2 
                          INNER JOIN usuarios u2 ON u2.id = v2.usuario_id 
                          WHERE u2.jefe_id = sup.id AND v2.etapa = 'Ganado' 
                          AND v2.fecha_venta BETWEEN @inicio AND @fin 
                          GROUP BY origen_campana ORDER BY COUNT(*) DESC LIMIT 1), 'N/A') as MejorCampana 
                FROM usuarios sup 
                LEFT JOIN usuarios vend ON vend.jefe_id = sup.id 
                LEFT JOIN ventas v ON v.usuario_id = vend.id AND v.fecha_venta BETWEEN @inicio AND @fin 
                WHERE sup.rol = 'Supervisor' 
                GROUP BY sup.id, sup.nombre", param);
            res.DesempenoPorSupervisor = sups.ToList();

            var can = await connection.QueryAsync<GraficoDatoDTO>(
                @"SELECT origen_campana as Etiqueta, SUM(monto) as Valor 
                FROM ventas WHERE etapa = 'Ganado' AND fecha_venta BETWEEN @inicio AND @fin 
                GROUP BY origen_campana", param);
        ...
        public async Task<DashboardSupervisorResponse> GetSupervisorData(int supervisorId, DateTime inicio, DateTime fin)
        {
            ...
            var rank = await connection.QueryAsync<VendedorRankingDTO>(
                @"SELECT u.nombre as Vendedor, 
                COUNT(CASE WHEN v.etapa = 'Ganado' THEN 1 END) as CantidadVentas, 
                COALESCE(SUM(CASE WHEN v.etapa = 'Ganado' THEN v.monto ELSE 0 END), 0) as MontoTotal, 
                COALESCE(AVG(CASE WHEN v.etapa = 'Ganado' AND v.csat > 0 THEN v.csat ELSE NULL END), 0) as PromedioCsat, 
                ROUND(COALESCE(SUM(CASE WHEN v.etapa = 'Ganado' THEN v.monto ELSE 0 END) / NULLIF(u.meta_mensual, 0) * 100, 0), 2) as MetaCumplidaPorcentaje 
                FROM usuarios u 
                LEFT JOIN ventas v ON v.usuario_id = u.id AND v.fecha_venta BETWEEN @inicio AND @fin 
                WHERE u.jefe_id = @supervisorId 
                GROUP BY u.id, u.nombre, u.meta_mensual 
                ORDER BY MontoTotal DESC", param);
            ...
            var tend = await connection.QueryAsync<GraficoDatoDTO>(
                @"SELECT DATE_FORMAT(v.fecha_venta, '%b %Y') as Etiqueta, 
                COALESCE(AVG(CASE WHEN v.csat > 0 THEN v.csat ELSE NULL END), 0) as Valor 
                FROM ventas v INNER JOIN usuarios u ON v.usuario_id = u.id 
                WHERE u.jefe_id = @supervisorId AND v.etapa = 'Ganado' 
                AND v.fecha_venta BETWEEN DATE_SUB(@fin, INTERVAL 5 MONTH) AND @fin 
                GROUP BY YEAR(v.fecha_venta), MONTH(v.fecha_venta), DATE_FORMAT(v.fecha_venta, '%b %Y') 
                ORDER BY YEAR(v.fecha_venta), MONTH(v.fecha_venta)", param);
            res.TendenciaCsat = tend.ToList();

            var det = await connection.QueryAsync<VentaDetalleDTO>(
                @"SELECT v.proyecto_nombre as Proyecto, c.nombre as Cliente, v.monto, v.etapa, 
                DATE_FORMAT(v.fecha_venta, '%d/%m/%Y') as FechaVenta, 
                DATE_FORMAT(v.fecha_ultimo_contacto, '%d/%m/%Y') as FechaUltimoContacto, 
                u.nombre as Vendedor, v.origen_campana as Origen, v.csat, 
                DATEDIFF(CURRENT_DATE(), v.fecha_ultimo_contacto) as DiasInactivo 
                FROM ventas v 
                INNER JOIN clientes c ON v.cliente_id = c.id 
                INNER JOIN usuarios u ON v.usuario_id = u.id 
                WHERE u.jefe_id = @supervisorId AND v.fecha_venta BETWEEN @inicio AND @fin 
                ORDER BY v.fecha_venta DESC", param);
        ...
        public async Task<DashboardVendedorResponse> GetVendedorData(int vendedorId, DateTime inicio, DateTime fin)
        {
            ...
            var emb = await connection.QueryAsync<GraficoDatoDTO>(
                @"SELECT etapa as Etiqueta, COUNT(*) as Valor 
                FROM ventas WHERE usuario_id = @vendedorId AND fecha_venta BETWEEN @inicio AND @fin 
                GROUP BY etapa", param);
            ...
            var list = await connection.QueryAsync<VentaDetalleDTO>(
                ...
                DATE_FORMAT(v.fecha_ultimo_contacto, '%d/%m/%Y') as FechaUltimoContacto, 
                DATEDIFF(CURRENT_DATE(), v.fecha_ultimo_contacto) as DiasInactivo, 
                v.origen_campana as Origen, v.csat 
                FROM ventas v 
                INNER JOIN clientes c ON v.cliente_id = c.id 
                WHERE v.usuario_id = @vendedorId AND v.fecha_venta BETWEEN @inicio AND @fin 
                ORDER BY v.fecha_ultimo_contacto ASC", param);
        ...
        public async Task<List<VentaDetalleDTO>> GetExportacionVentas(int id, string rol, DateTime inicio, DateTime fin)
        {
            ...
            if (rol == "Supervisor") sql += " AND u.jefe_id = @id";
            else if (rol == "Vendedor") sql += " AND u.id = @id";
